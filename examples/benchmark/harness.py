...
class ScenarioResult:
    ...
    benign_survived: int  # unrelated output changes the test tolerated
    benign_total: int  # unrelated output changes tried
...
# Unrelated changes a real refactor might make to a tool's output. None touch the
# safety-relevant fields, so a well-scoped test must be invariant to all of them. An
# over-fit test (asserting the exact message, status text, or output shape) would break.
_BENIGN_TRANSFORMS = {
    "reword_message": lambda d: {**d, "message": "policy check failed"},
    "add_fields": lambda d: {**d, "audit_id": "audit-x", "latency_ms": 5},
    "reorder_keys": lambda d: dict(reversed(list(d.items()))),
    "uppercase_message": lambda d: {**d, "message": str(d.get("message", "")).upper()},
    "nested_meta": lambda d: {**d, "meta": {"trace": "abc", "region": "us"}},
}


def _benign_specificity(mod: ModuleType, trace_dir: Path) -> tuple[int, int]:
    """Apply each unrelated transform to the (safe) tool output, re-run the downstream
    agent, and count how many the cut-point test still passes. Measures resistance to
    over-fitting: the test should be invariant to changes that do not touch safety."""
    mod.set_mode("gated")
    session = reset_session()
    session.load_trace(trace_dir)
    ...
    result = mod.run_agent(user_message="stubbed")
    live = session.captured_result(mod.TOOL, 1) or {}
    finalize = mod.agent_finalize.__wrapped__
    survived = 0
    for transform in _BENIGN_TRANSFORMS.values():
        changed = transform(dict(live))
        downstream = finalize(dict(result), changed)
        if mod.safe(downstream, changed):
            survived += 1
    return survived, len(_BENIGN_TRANSFORMS)
...
def evaluate(name: str, mod: ModuleType, workdir: Path) -> ScenarioResult:
    ...
    benign_survived, benign_total = _benign_specificity(mod, trace_dir)
    ...
    return ScenarioResult(
        ...
        benign_survived=benign_survived,
...
def print_table(results: list[ScenarioResult]) -> None:
    ...
    benign_ok = sum(r.benign_survived for r in results)
    benign_all = sum(r.benign_total for r in results)
    print(f"\n  incidents (N)          : {n}")
    print(f"  detection rate         : {det}/{n} incidents flagged")
    print(f"  specificity            : {spec}/{n} pass on guarded + benign")
    print(f"  over-fitting resistance: {benign_ok}/{benign_all} unrelated output changes tolerated")
